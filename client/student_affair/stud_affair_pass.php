...
$stmt=$pdo->prepare("SELECT sa_password from student_affairs where sa_id=:id");
$stmt->execute(array(":id"=>$_SESSION["said"]));
...
if(isset($_POST["btnsubmit"])){
  ...
  if($oldPassoword != $row["sa_password"]){
...
function changePassword($nwpwd){
  ...
  $sql="UPDATE student_affairs set sa_password=:nwpwd where sa_id=:id";
  $stmt1=$pdo->prepare($sql);
  $stmt1->execute(array(":nwpwd"=>$check,":id"=>$_SESSION["said"]));
