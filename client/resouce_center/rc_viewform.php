...
if ($row) {
  ...
  header("location:../resouce_center/rcindex.php");
...
function getStatus($reqId){
  global $pdo;
  $ispending=false;
  $sql=$pdo->prepare("SELECT status_id from rc_process where req_id=:id");
...
$checkRcSql = "SELECT fp.status_id, r.serv_id FROM finance_process fp
              INNER JOIN request r ON fp.req_id = r.req_id
              WHERE fp.req_id = :id"; //! Check if status_id in finance_process is 8

$checkRcStmt = $pdo->prepare($checkRcSql);
$checkRcStmt->execute(array(":id"=>$reqId));
$fpData = $checkRcStmt->fetch(PDO::FETCH_ASSOC);



  $sql ="UPDATE rc_process SET  status_id = 8, date_process = :dt, rc_id=:rid WHERE req_id = :id";
  $stmt1 = $pdo->prepare($sql);
  $stmt1->execute(array( ":dt" => $todayDate ,":rid"=>$_SESSION['rcid'] ,":id" => $reqId));





if ($fpData['status_id'] == 8) {

    switch ($fpData['serv_id']) {
        case 1:
        case 4:
        case 5:
        case 6:
  ...
  <div class="container">
    <h1 class="form-title">Student Request Form</h1>
      ...
      <div class="main-user-info">
        ...
        <a href="../resouce_center/rcindex.php"class="btn btn-primary btn-lg">Back Home</a>
  </div>
  <!-- Modal START HERE -->
    ...
    <div class="modal-dialog">
      <div class="modal-content">
        ...
        <div class="modal-body">
          <form method="POST"action="rcProcessClearance.php">
